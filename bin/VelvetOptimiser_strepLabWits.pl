...
# Modfications made by Scott Hazelhurst 2021
...
#print $OUT "HI BEN!!!!!!!\npstringg variable: " . $assembliesObjs{$bestId}->{pstringg}. "\n";
...
#print $OUT "Hi again BEN!!\n" . $assembliesObjs{$bestId}->toString() if $verbose;
if($finaldir eq "."){
...
sub covCutoff{
	...
	`touch LOW_COVERAGE`;
        print STDERR "Minimum specified coverage cutoff is higher than the expected coverage. Please choose a minimum coverage cutoff smaller than $maxc and re-run.\n";
	exit;
    }
   ...
   #re-write the pstringg with the new velvetg command..
    my $vg = $ass->{pstringg};
    if($vg =~ /cov_cutoff/){
        $vg =~ s/cov_cutoff\s+\d+/cov_cutoff $b/;
    }
    else {
        $vg .= " -cov_cutoff $b";
    }
    $ass->{pstringg} = $vg;
    print $OUT "HI BEN. COV_CUTOFF VAR: " . $ass->toStringNoV();
    return 1;
